...
document.addEventListener('DOMContentLoaded', () => {
        ...
        direct: {
              ...
              <div class="svg-wrap">
                  <svg id="direct-scene" viewBox="0 0 960 540">
                          <defs>
                              ...
                              <g id="direct-switch"><line x1="-60" y1="0" x2="60" y2="0" class="direct-wire"/><line id="direct-bridge" x1="-10" y1="0" x2="10" y2="0" class="direct-gate" opacity="1"/><circle cx="0" cy="0" r="6" fill="#ef4444"/><text x="0" y="-38" text-anchor="middle" class="direct-label">Schakelaar</text><text x="0" y="28" text-anchor="middle" class="direct-small">Open = geen stroom</text></g>
            ...
            init: function() {
                ...
                function updateCalculations(){
                    const U = parseFloat(uSlider.value) || 0;
                    const R = parseFloat(rSlider.value) || 1;
                    const I = running ? U / R : 0;
                    const P = running ? U * I : 0;
                    
                    uVal.textContent = `${U.toFixed(1)} V`;
                    rVal.textContent = `${R.toFixed(0)} Ω`;
                    iOut.textContent = I.toFixed(3);
                    pOut.textContent = P.toFixed(2);
                    
                    const pNorm = Math.min(1, P / 50); // Normalize power for visual effect
                    glow.setAttribute('opacity', (0.12 + 0.88 * pNorm).toFixed(3));
                ...
                function step(ts){
                    ...
                    if (running) {
                        totalEnergy += P * dt;
                        eOut.textContent = totalEnergy.toFixed(1);
                        
                        const speed = I * 200; // Visual speed based on current
                        setChargeCount(Math.round(I * 20)); // Electron density based on current
    ...
    function displayExplanation(type) {
        ...
        if (explanations[type]) {
            explanationContainer.innerHTML = explanations[type].html;
            explanations[type].init();
    ...
    // Default to show first explanation
    ...
    // --- Answer Checking Logic for Metaphor Questions ---
    explanationContainer.addEventListener('click', function(event) {
        if (event.target.matches('.answer-btn')) {
            const isCorrect = event.target.dataset.correct === 'true';
            const feedbackEl = this.querySelector('#answer-feedback');
            ...
            if (isCorrect) {
                ...
                feedbackEl.textContent = '✅ Correct! Goed begrepen.';
                feedbackEl.className = 'correct';
            } else {
                event.target.classList.add('incorrect');
                feedbackEl.textContent = '❌ Dat is niet helemaal juist. Denk nog eens na over de analogie.';
                feedbackEl.className = 'incorrect';
                // Highlight the correct button after a short delay
                setTimeout(() => {
                    const correctBtn = this.querySelector('.answer-btn[data-correct="true"]');
